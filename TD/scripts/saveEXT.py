...
try:
    # import td
    from td import op, run, parent, ui, project, tdu # type: ignore
except ModuleNotFoundError:
    from vvox_tdtools.td_mock import op, run, parent, ui, project, tdu
...
class ExternalFiles:
    ...
    def __init__(self, my_op):
        ...
        init_msg 				= f"Save init from {my_op}"
    ...
    def Prompt_to_save(self, current_loc):
        ...
        msg_box_msg 					= f"Replacing External\n\nYou are about to overwite an exnteral TOX: {current_loc.name}"
        ...
        if current_loc == '/':
            ...
            return None
        if 'NOTOX' in current_loc.tags:
            while current_loc.name != 'root':
                if 'NOTOX' in current_loc.tags:
                    current_loc = current_loc.parent()
                    if 'NOTOX' not in current_loc.tags:
                        break
                else:
                    break
            if current_loc.name == 'root':
                return None
    ...
    def Save_over_tox(self, current_loc):
        ...
        log_msg 		= f"{current_loc} saved to {project.folder}/{external_path}"
    ...
    def Save_tox(self, current_loc):
        ...
        start_path = pathlib.Path(project.folder) / 'td-modules'
        if not start_path.exists():
            start_path = project.folder

        save_loc 		= ui.chooseFolder(title="TOX Location", start=start_path)
        ...
        # create and print log message
        log_msg 		= f"{current_loc} saved to {project.folder}/{tox_path}"
        self.Logtotextport(log_msg)
        return
    ...
    def Reload_ext_dat(self, external_file):
        ...
        file_path 			= f"{project.folder}/{external_file}"
        ...
        for each_op in self.Op_finder.cols(2)[0][1:]:
            ...
            if os.path.isabs(each_op_path):
                ...
                each_op_path = f"{project.folder}/{each_op_path}"
